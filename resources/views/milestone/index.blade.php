...
@section('content')
    <div class="container">
        <x-table>
            <x-slot name="titolo">
                @auth
                    @if($sottoProgetto->responsabile_id == Auth::user()->id)
            ...
            <x-slot name="righe">
                ...
                @foreach ($milestones as $milestone)
                    <x-tr class="@if($loop->index%2==0) bg-gray @else bg-white @endif">
                        <x-slot name="body">
                            <x-td>
                                <x-slot name="body"> <a href="{{ route('milestone.show', [$milestone->sotto_progetto->id,$milestone->id]) }}">{{ $milestone->descrizione }}</a></x-slot>
                            </x-td>
