...
@section("content")
    <div class="container">
        @if(!$ricercatori->isEmpty())
            <x-table>
                <x-slot name="titolo">
                    <x-button class="float-right"> <a href="{{ route('progetti.add_ricercatore', compact("progetto")) }}"><i class="lni lni-plus"></i> Aggiungi ricercatore</a></x-button>
                        ...
                        <x-tr class="@if($loop->index%2==0) bg-gray @else bg-white @endif">
                            <x-slot name="body">
                                ...
                                <x-td>
                                    <x-slot name="body">
                                        <form method="POST"
                                              action="{{ route('progetti.remove_ricercatore', ["progetto" => $progetto, "ricercatore" => $ricercatore] ) }}"
                                              id="delete_progetto"
