        ...
        @if(!$sottoProgetti->isEmpty())
            <x-table>
                <x-slot name="titolo">
                    @if(app('request')->input('progetto') !== null)
                        @auth
                            ...
                            @endif
                        @endauth
                        <h2 class=" testo titolo grande">Elenco dei sottoprogetti del progetto {{ \App\Models\Progetto::find(app('request')->input('progetto'))->titolo }}</h2>
                ...
                <x-slot name="colonne">
                    ...
                    @auth
                        @if(Auth::user()->hasRuolo("manager") || Auth::user()->hasRuolo("responsabile"))
                    ...
                    @foreach ($sottoProgetti as $sottoProgetto)
                        <x-tr class="@if($loop->index%2==0) bg-gray @else bg-white @endif">
                            <x-slot name="body">
                                <x-td>
                                    <x-slot name="body">
                                        <a href="{{route("sottoprogetti.show", $sottoProgetto)}}">{{$sottoProgetto->titolo}}</a>
                                    </x-slot>
                                </x-td>
                                <x-td>
                                ...
                                @auth
                                    @if((Auth::user()->hasRuolo("responsabile") || Auth::user()->hasRuolo("manager")))
                                        <x-td>
                                            <x-slot name="body">
                                                @if(Auth::user()->hasRuolo("manager"))
                                                ...
                                                @elseif(Auth::user()->hasRuolo("responsabile") && $sottoProgetto->responsabile_id == Auth::user()->id)
